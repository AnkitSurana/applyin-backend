...
CURRENT_POLICY_VERSION = settings.PRIVACY_POLICY_VERSION if hasattr(settings, "PRIVACY_POLICY_VERSION") else "2026-06-01"


def _hash_ip(ip: str) -> str:
    if not ip:
        return ""
    salt = (settings.RAZORPAY_KEY_SECRET or "applyin-salt").encode()  # any server secret as salt
