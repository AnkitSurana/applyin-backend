...
async def payment_callback(
    ...
    # Atomic claim (pending -> paid). Razorpay fires both this callback and the
    # webhook for the same payment, so only the call that actually flips the row
    # grants credits; the other just confirms. Prevents double-crediting.
    ...
    if not claim.data:
        # Already credited by the webhook (or a prior callback). Confirm only.
    ...
    try:
        db.table("credit_ledger").insert({
            ...
            "reason": "purchase",
            "meta": {
                "package_id": won["package_id"],
                "razorpay_payment_link_id": razorpay_payment_link_id,
                "razorpay_payment_id": razorpay_payment_id,
            }
        }).execute()
    except Exception as e:
        # Grant failed after claiming; revert so the webhook/retry can re-process.
        logger.error(f"Credit grant failed after claim for {razorpay_payment_link_id}; reverting: {e}")
        db.table("credit_orders").update({"status": "pending"}) \
            .eq("razorpay_order_id", razorpay_payment_link_id).execute()
        return HTMLResponse(content=_result_page(False, "Payment received but crediting failed. Please contact support."))

    logger.info(f"Credits added: +{won['credits']} → user {won['user_id']}")
    return HTMLResponse(content=_result_page(True, f"{won['credits']} credits added! Return to LinkedIn.", won["credits"]))
