...
Reads retain_days from the retention_policy table (single source of truth set in
schema_compliance.sql) and deletes rows older than that in each category.

Run on a schedule. Two options:
  1. As a small cron job (Render Cron / GitHub Actions) calling this script.
  2. As pg_cron inside Supabase (see schema_compliance.sql for SQL equivalents).
